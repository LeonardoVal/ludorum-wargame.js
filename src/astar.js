...
var Graph = exports.Graph = declare({
  ...
  constructor: function Graph(gridIn, options) {
    options = options || {};
    this.nodes = [];
    this.diagonal = !!options.diagonal;
    this.grid = [];
    this.trueMetric= options.trueMetric|| false;
    this.unitSize=options.unitSize|| 1;
    this.astar=new astar();
    var node,x,y,valueOfNode,row;
    for (x = 0; x < gridIn.length; x++) {
      this.grid[x] = [];
      for (y = 0, row = gridIn[x]; y < row.length; y++) {
          valueOfNode=row[y]===0 ? 1 : 0;
          if(this.trueMetric && this.trueMetric[x][y]<this.unitSize){
              valueOfNode=0;
          }
